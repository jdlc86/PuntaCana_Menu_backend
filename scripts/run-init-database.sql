-- Creating a new script to initialize the database with all tables and predefined categories
-- This will create the menu_categories table with predefined categories: Appetizers, Main Courses, Desserts, Beverages
...
DROP TABLE IF EXISTS public.bill_requests CASCADE;
...
-- Create menu_categories table with predefined categories
...
-- Create additional tables for the restaurant system
...
('Welcome!', 'Welcome to our restaurant ordering system', 'info'),
('Special Offer', 'Get 20% off on all desserts today!', 'success'),
('Kitchen Notice', 'Pasta dishes may take extra time due to high demand', 'warning');

-- Insert sample reviews
INSERT INTO public.reviews (table_id, customer_name, rating, comment) VALUES
(1, 'Alice Johnson', 5, 'Excellent food and service!'),
(2, 'Bob Wilson', 4, 'Great atmosphere, food was good');
...
CREATE POLICY "Allow all operations on announcements" ON public.announcements FOR ALL USING (true);
CREATE POLICY "Allow all operations on waiter_calls" ON public.waiter_calls FOR ALL USING (true);
CREATE POLICY "Allow all operations on bill_requests" ON public.bill_requests FOR ALL USING (true);
